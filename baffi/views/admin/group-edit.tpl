...
<form class="form-horizontal" action="{$SCRIPT_NAME}?action=submit" method="POST">
    ...
    <div class="control-group">
        <label class="control-label" for="regexmatchonly">Pre Filter Binaries</label>
        <div class="controls">
            {html_radios id="regexmatchonly" name='regexmatchonly' values=$yesno_ids output=$yesno_names selected=$group.regexmatchonly separator=''}
            <span class="help-block">Enabling pre-filter will only store binaries and parts in the database when they match release regex. Disabling will store all incoming binaries and parts until header retention met.</span>
        </div>
    </div>
